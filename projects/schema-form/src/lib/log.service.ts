...
export class DefaultLogService extends LogService {
  logLevel: any /* should be of type `LogLevel` but AOT fails with : 'Error encountered in metadata generated for exported symbol 'DefaultLogService':"Could not resolve type LogLevel." */;

  constructor() {
    const logLevel = inject(LOG_LEVEL, {optional: true});
  ...
  warn = (!this.isWarnEnabled() ? () => {
  } : console.warn)
  error = (!this.isErrorEnabled() ? () => {
  } : console.error)
  log = (!this.isLogEnabled() ? () => {
  } : console.log)
}
